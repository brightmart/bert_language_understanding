...
tf.app.flags.DEFINE_string("mask_lm_source_file","./data/xxx_20181022_train.txt","source file to generate data for masked language model") #./data/cail2018_bi.json
...
tf.app.flags.DEFINE_boolean("is_pretrain",True,"is training.true:tranining,false:testing/inference")
...
#num_classes_lm=FLAGS.vocab_size
...
def main(_):
    ...
    X_mask_lm, y_mask_lm, mask_lm_dict,mask_lm_dict_valid=mask_language_model(FLAGS.mask_lm_source_file,FLAGS.data_path,index2word,max_allow_sentence_length=FLAGS.max_allow_sentence_length)
    ...
    with tf.Session(config=gpu_config) as sess:
        ...
        list_indices = [i for i in range(FLAGS.max_allow_sentence_length)]

        mask_lm_dict_index={} # for generate index
        for k_index,v_list in mask_lm_dict.items():
            mask_lm_dict_index[k_index]=[i for i in range(len(v_list))]
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                index = random.sample(list_indices, 1)[0]
                batch_index=random.sample(mask_lm_dict_index[index], batch_size)
                list_tuple_mask_lm=mask_lm_dict[index]
                train_X_mask_lm=[list_tuple_mask_lm[0][index] for index in batch_index]
                train_y_mask_lm=[list_tuple_mask_lm[1][index] for index in batch_index] #list_tuple_mask_lm[1][batch_index]
                train_postion_mask_lm=index
                print("#######model.input_y_mask_lm.train_y_mask_lm[start:end]:",train_y_mask_lm[start:end].shape,";input_x_mask_lm:",train_X_mask_lm.shape)
                feed_dict = {model.input_x_mask_lm: train_X_mask_lm,
                             model.input_y_mask_lm: train_y_mask_lm,model.train_position_mask_lm:train_postion_mask_lm,
                             model.dropout_keep_prob: FLAGS.dropout_keep_prob}
                current_loss_lm,lr,l2_loss,_=sess.run([model.loss_val_lm,model.learning_rate,model.l2_loss,model.train_op],feed_dict)
                loss_total_lm,counter=loss_total+current_loss_lm,counter+1
                if counter %30==0:
                    print("Learning rate:%.3f\tLoss_lm:%.3f\tCurrent_loss_lm:%.3f\tL2_loss%.3f\t"%(lr,float(loss_total_lm)/float(counter),current_loss_lm,l2_loss))
                if start!=0 and start%(3000*FLAGS.batch_size)==0:
                    loss_valid, f1_macro_valid, f1_micro_valid= do_eval(sess, model, valid,num_classes,label2index,mask_lm_dict_index,batch_size,mask_lm_dict_valid)
...
def do_eval(sess,model,valid,num_classes,label2index,mask_lm_dict_index,batch_size,mask_lm_dict):
    """
    ...
    batch_index = random.sample(mask_lm_dict_index[index], batch_size)
    list_tuple_mask_lm = mask_lm_dict[index]
    valid_X_mask_lm = list_tuple_mask_lm[0][batch_index]
    valid_y_mask_lm = list_tuple_mask_lm[1][batch_index]
    valid_postion_mask_lm = index
    ###############
    ...
    for start,end in zip(range(0,number_examples,batch_size),range(batch_size,number_examples,batch_size)):
        feed_dict = {model.input_x: valid_x[start:end],model.input_y:valid_y[start:end],model.input_x_mask_lm: valid_X_mask_lm[start:end],
                     model.input_y_mask_lm: valid_y_mask_lm[start:end],model.train_position_mask_lm: valid_postion_mask_lm,
                     model.dropout_keep_prob: FLAGS.dropout_keep_prob}
        curr_eval_loss, logits= sess.run([model.loss_val_lm,model.logits_lm],feed_dict) # logits：[batch_size,label_size]
